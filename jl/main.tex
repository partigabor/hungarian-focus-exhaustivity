...
To address the theoretical debate, and in light with recent experimental studies \citep{onea_2009_hungarian,gerocs_2014_exhaustivity,kaldi_2021_hungarian_structural}, we designed a visual-world experiment with eye-tracking to differentiate interpretations associated with the PVF structures. Specifically, we were curious about Hungarian speakers' dominant interpretation of the unmodified, simple PVF sentence (e.g., \ref{ex:pvf}), in comparison with the \textit{only}--exhaustive (\ref{ex:only}) and the \textit{amongst others}--contrastive conditions (\ref{ex:amongst_others}). 
