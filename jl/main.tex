...
{\caption{Distribution of chosen image type (A vs B) per condition.}}
\label{fig:choice}
\end{figure}
...
The behavioural results showed that participants overwhelmingly interpreted the unmodified pre-verbal focus (PVF) sentences like exhaustive \textit{only}-sentences, choosing single-object type A images (see Figure~\ref{fig:choice}, Table~\ref{tab:choice}). However, in the contrastive condition (\textit{among others}-sentences) people preferred the multi-object type B images, exhibiting a tendency toward non-exhaustive readings, but with almost 37.7\% of the choices -- somewhat unexpectedly -- still being cast on the exhaustive interpretation type A images, which suggests that even with the presence of a phrase that licenses a contrastive reading, the exhaustive effect is still quite dominant. This is clear evidence that the PVF construction is not necessarily interpreted only exhaustively, a contrastive reading can be easily be teased out.
...
Across eye-tracking measures, linear mixed-effects models (random intercepts for participant and item) were fit to log-transformed outcomes ($\log(1+x)$) with fixed effects of \textit{Condition} (\textit{exclusive}, \textit{focus}, \textit{contrastive}), \textit{Choice type} (A = single-object choice, interpreted as an \textit{exhaustive} reading; B = multi-object choice, interpreted as a \textit{contrastive} reading), and their interaction. For \textit{total dwell time}, likelihood-ratio tests indicated that including the Condition $\times$ Choice type interaction significantly improved model fit relative to the additive model ($\chi^2(2)=12.23$, $p=.0022$). In the full model, Type III tests revealed significant effects of Condition ($\chi^2(2)=12.53$, $p=.0019$) and Choice type ($\chi^2(1)=11.85$, $p=.00058$), qualified by a significant interaction ($\chi^2(2)=12.29$, $p=.0021$). Follow-up estimated marginal means (Tukey-adjusted) showed that for exhaustive (type A) trials, the \textit{contrastive} condition elicited greater dwell time than both \textit{exclusive} ($\Delta=0.188$, $p=.0013$) and \textit{focus} ($\Delta=0.140$, $p=.022$), whereas \textit{exclusive} and \textit{focus} did not differ. In contrast, for contrastive (type B) trials, condition differences were not reliable (all $p\ge .069$). Comparing choice types within each condition further showed that in the \textit{exclusive} condition, contrastive (type B) choices were associated with longer dwell time than exhaustive (type A) choices ($\Delta=0.444$, $p=.0006$), with no corresponding differences in the other conditions.
...
% A converging pattern was observed for \textit{total fixation count}. Condition improved fit relative to a model with only Choice type ($\chi^2(1)=6.53$, $p=.0106$), and the Condition $\times$ Choice type interaction further improved fit ($\chi^2(2)=6.74$, $p=.034$). In the full model, Condition ($\chi^2(2)=12.33$, $p=.0021$) and the interaction ($\chi^2(2)=6.76$, $p=.034$) were significant, while the main effect of Choice type was not ($\chi^2(1)=3.35$, $p=.067$). Post hoc comparisons indicated that for exhaustive (type A) trials, the \textit{contrastive} condition yielded more fixations than \textit{exclusive} ($\Delta=0.158$, $p=.0016$) and \textit{focus} ($\Delta=0.131$, $p=.0107$), whereas no condition differences emerged for contrastive (type B) trials (all $p\ge .483$). Taken together, both measures suggest that condition-driven processing differences are concentrated in trials culminating in an exhaustive (single-object) choice, consistent with the contrastive manipulation selectively increasing processing demands when participants resolve the stimulus exhaustively.
