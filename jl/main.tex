...
\begin{itemize}
  \item[\bigstar] \textbf{Concerns about stimulus design and procedure.} \hl{The exhaustive and unmodified conditions begin with expressions such as ``regarding drinks/fruits'', which evoke alternatives and may bias contrastive or exhaustive interpretations.} The contrastive condition also differs from the other two in sentence structure, discourse framing, and lexical content, creating a possible confound: observed differences may reflect sentence context, lexical material, or \textbf{length} rather than focus interpretation.
  \begin{itemize}
      \item Length is same, lexical material is same (?), discourse framing and sentence structure maybe.
  \end{itemize}
  
  \item \textbf{Linguistic-material decisions need justification.} Explain the significance of verbal prefixes, definite versus indefinite conjugation, and countable versus uncountable nouns so decisions are comprehensible to non‑Hungarian readers. Since the introduction presents preverbal focus partly via separation of the verbal prefix from the verb, clarify how stimuli without verbal prefixes fit the design.
  \item \textbf{Clarify what a ``submission'' means.} State whether participants could complete more than one part of the experiment, whether submissions occurred in single or multiple sittings, and how multiple sittings were counted.
  \item \textbf{Model repeated submissions explicitly.} If the same participant could contribute multiple submissions, this must be modelled, especially if participants might have seen the same items in different conditions, because repeated exposure affects reading and reaction times.
  \item \textbf{Describe attention checks.} Provide details about the attention‑check items used.
  \item \textbf{Clarify counterbalancing.} The phrase ``the image types were counterbalanced throughout the trials'' should be made explicit (e.g., left/right position of single‑object vs multi‑object images).
  \item \textbf{Report timing of demographic collection.} State when and how participant age and gender were collected.
\end{itemize}
...
\subsection{Terminology and typos \bigstar}
...
\begin{itemize}
  ...
  \item \textbf{Author stance and expertise.} I should note here that I have no specific expertise on any aspect of Hungarian and I have no prior theoretical commitment to any particular analysis of the phenomena in question; that is to say, I'm very willing to be persuaded of the correctness of the authors' analysis, but I have not been persuaded yet.
...
{\caption{Image pair for the sentence in examples (\ref{ex:stimuli_exhaustive}--\ref{ex:stimuli_contrastive}). The left image (type A) corresponds to the exhaustive reading, while the right image (type B) corresponds to the contrastive reading.}
\label{fig:stimuli_8}}
\end{figure}
...
\hl{Stimuli, full results, replication data and code} can be found at \url{https://osf.io/j9abg/overview?view_only=484a4f21a5cb4e51b4b826f970e096a0}.
