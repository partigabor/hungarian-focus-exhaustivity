\documentclass[a4paper, ruler]{CUP-JNL-LIN}%[ruler,draftrules]
...
\abstract{Whether the Hungarian pre-verbal focus (PVF) construction encodes exhaustivity semantically or derives it through pragmatic inference remains a central debate in generative linguistics. While
semantic accounts posit an inherent exhaustive operator, pragmatic models treat exhaustivity as a cancellable implicature. We address this controversy through a multi-modal visual-world study (n=52) combining sentence--picture matching, self-paced reading, and webcam-based eye tracking. Participants read sentences in three conditions -- explicitly exhaustive (\textit{csak} `only'), unmodified PVF, and contrastive (\textit{többek között} `amongst others') -- and chose between a single-object (exhaustive) and a multi-object (contrastive) image. Behavior results reveal a robust exhaustive default for unmodified PVF (96.1\%), near-identical to the `only' condition. In the contrastive condition, 62.3\% of choices were the multi-object image, showing that a non-exhaustive reading is genuinely available. However, reaction times and reading-time data uncover a significant processing cost for unmodified PVF compared to \textit{csak} `only', indicating a measurable inferential step required to derive exhaustivity. Gaze-fixation data further demonstrates that without explicit modifiers, exhaustive and contrastive interpretations remain initially plausible, supporting a semantically underspecified representation. This study reconciles 40 years of conflicting theoretical accounts. Our findings provide a processing-based model for the syntax--pragmatics interface, suggesting that robust behavioral defaults can mask underlying inferential complexity.}
...
\keywords{Hungarian, syntax, focus, exhaustivity, contrastivity, visual-world paradigm, sentence--picture matching, self-paced reading, eye-tracking}
...
\begin{itemize}
  \item \textbf{Justify winsorisation choices.} The procedure that replaces values above mean + 2 SD with the global mean is unusual and may distort data; a standard winsorisation replaces extreme values with the threshold value itself.
  \item \textbf{Resolve notation inconsistencies.} Clarify whether only high outliers or both high and low outliers were treated, and standardize notation (SD vs $\sigma$), specifying whether sample (SD) or population ($\sigma$) standard deviation was used.
  \item \textbf{Define AOIs relative to image type.} If image‑type position varied, define Areas of Interest (AOIs) with reference to image type rather than screen half.
\end{itemize}

\subsection{Statistical Analysis}

\begin{itemize}
  \item \textbf{Expand model descriptions.} State explicitly the models for picture choice, sentence reading times, region‑level reading times, and decision reaction times, and specify the contrast coding used for all categorical predictors (treatment vs sum coding).
  \item \textbf{Analyze choice data statistically.} Add a statistical analysis for picture choice. For a binary outcome, use a generalized linear mixed‑effects model (binomial family, logit link) with sentence condition as a fixed effect and random effects for participants and items.
  \item \textbf{Control for sentence length in reading‑time analyses.} Whole‑sentence reading times are confounded by sentence length and lexical repetition (e.g., \emph{csak} is one syllable in the exhaustive condition). If retained, include syllable or character count as a covariate; note that lexical repetition may still confound results.
  \item \textbf{Region‑level confounds.} Region 5 is confounded with word length, lexical identity, and repetition across conditions; avoid drawing strong theoretical conclusions from this region as currently analysed.
  \item \textbf{Interpret Region 6 carefully.} Regions 6 and 7 are more comparable across conditions but may show spillover effects. Correct the reported interpretation: a negative coefficient for Choice B (with Choice A as reference) on log reading time indicates shorter, not longer, reading times for Choice B in the reference condition.
  \item \textbf{Account for imbalance in picture choice.} Because multi‑object choices are rare in some conditions and frequent in others, choice effects are unreliable; picture choice is an outcome of interpretation and not independent of sentence condition.
  \item \textbf{Be cautious with pairwise comparisons.} Do not base substantive conclusions on separate pairwise contrasts when the Sentence Condition $\times$ Choice Type interaction is non‑significant and cell sizes are imbalanced; a non‑significant contrast in small cells is not evidence of absence.
  \item \textbf{Reconsider eye‑tracking measures.} Webcam‑based eye tracking is less precise; total dwell time and fixation count correlate strongly with decision time and may add little beyond reaction‑time data. If claiming simultaneous availability of interpretations, use time‑sensitive measures tied to competition (looks to single vs multi‑object over time, first fixation, switching patterns, target‑preference curves) rather than total dwell time alone.
\end{itemize}

\subsection{Discussion}

\begin{itemize}
  \item \textbf{Current data do not support clear conclusions.} Given the methodological and analytical issues above, no clear conclusions about preverbal focus interpretation can be drawn from the present data.
  \item \textbf{Minimum revisions required.} At minimum, add a statistical analysis of picture choice; specify all models and contrast coding; reanalyse reading times with length and lexical controls; correct the Region 6 interpretation; avoid conclusions based on unequal pairwise comparisons when interactions are non‑significant; and clarify what eye‑tracking measures contribute beyond decision time.
  \item \textbf{Recommended ideal step.} Include a control experiment or baseline condition without sentential structure to test whether the picture‑choice pattern arises from the overt modifiers alone. Without such a control, it is unclear whether the study tests preverbal focus interpretation or interpretation of the modifiers used.
\end{itemize}

\subsection{Terminology and typos}

Mostly done... \bigstar

\begin{itemize}
  \item \textbf{Check tables and captions.} Ensure consistency in terminology, especially ``fixation times'' versus ``fixation counts.''
\end{itemize}

\section{Reviewer 2}

\begin{itemize}
  \item \textbf{Overall appraisal.} I found this an interesting paper, and appreciated (up to a point) the thoroughness with which the experimental results were analysed; however, I did have some concerns about the \textbf{theoretical underpinnings} of the work and the operationalisation involved.
  
  \item \textbf{Core concern and predicted consequences.} The essence of my concern is that, in summarising the results of this paper as a rebuttal of the strong semantic exhaustivity view, the authors argue (p.31) that that view yields three predictions that are not borne out by these data:
    \begin{itemize}
      \item ``(i) non-exhaustive readings should be unavailable or highly marked,''
      \item ``(ii) the unmodified PVF should be processed identically to the only-condition,''
      \item ``(iii) co-occurrence with \emph{többek között} `amongst others' should be degraded or induce a strong semantic conflict.''
    \end{itemize}
  \item \textbf{Authors' counterevidence summary.} The counterevidence for each point respectively is summarised as: ``Non-exhaustive readings were chosen in over a third of contrastive trials, the unmodified condition was processed more slowly than the explicitly marked exhaustive condition, and the contrastive condition was grammatically acceptable and produced meaningful, interpretable results.''
  \item \textbf{Highlighted emphases.}
    \begin{itemize}
      \item The phrase \textbf{``theoretical underpinnings''}.
      \item The quoted predictions beginning with ``(i) non-exhaustive readings...'' and ending with ``...semantic conflict.''
    \end{itemize}
\end{itemize}

\begin{itemize}
  \item \textbf{My respective worries about these arguments are as follows.}
  \item \textbf{(i)} There's very little evidence here of participants spontaneously entertaining a non-exhaustive reading unless they are obliged to do so by the addition of ``amongst others'', the explicit utterance of which could be argued to be a highly marked kind of occurrence.
  \item \textbf{(ii)} This seems like an unmotivated claim, as far as the processing signature is concerned. I agree that the theory would predict that the same interpretation is ultimately arrived at, much like the case alluded to of ``only some'' versus ``some'' in scalar implicature-supporting contexts. But there doesn't seem to be any very convincing reason to posit identical processing pathways. To articulate one particular concern, the explicit presence of ``csak'' might signal exhaustivity in some way that causes the reader to expect (or at least be less surprised by) the presence of pre-verbal focus, i.e. the fact that the next item is not a verb. If that were to happen — and I take it that this is pretty routine, as hearers routinely entertain rich probabilistic expectations about what will follow — there doesn't seem to be any reason to expect reading times and resulting gaze behaviour to be aligned between the PVF and ``only'' conditions.
\end{itemize}


\begin{itemize}
  \item \textbf{(iii)} There doesn't seem to be any evidence in this paper that the co-occurrence with \emph{többek között} isn't degraded and doesn't introduce a strong semantic conflict. I take it that it's the authors' judgment, as well as that of at least some prior work, that these sentences are essentially OK, but then this looks like a stronger argument against the semantic exhaustivity view than anything provided in this paper.
  \item Unless I misunderstand the task here, arguing that the contrastive condition is acceptable because it produces 'meaningful, interpretable results' strikes me as very weak; participants are asked to choose which picture best matches what they've read, and an ability to do that doesn't seem like compelling evidence that what they've read is grammatical or coherent. (To labour the point, if I showed participants a red square and a blue square and said 'click on the red blue square', or indeed 'click on the red bluer square', I'd expect them to respond in some way, but that wouldn't evidence that the sentences were either grammatical or non-self-contradictory.)
  \item One thing that struck me in general, in terms of disentangling the competing theoretical analyses, is that \textbf{exhaustivity is usually taken to be relative to some restricted domain}; certainly if we consider an utterance like 'Anna ate only a banana', we don't interpret that in real life as an assertion that Anna has never previously eaten anything except this one banana.
\end{itemize}

\begin{itemize}
  \item So it wasn't wholly clear to me whether there was any real contradiction between the PVF having exhaustivity in its semantics and it being compatible with a continuation which acknowledges that, relative to a different domain, the claim arising from the PVF is not truly exhaustive. The question seems to be how the putatively semantic content is in fact supposed to relate to content which ostensibly contradicts it, and how we get from there to conclusions about whether that content is in fact semantic at all.
  \item As a minor point, I did also wonder how participants are treating the pictorial representations in this study. As described here, participants are instructed to ``select the image that best matched the meaning of the sentence they just read'' (p.9). But as exemplified here, the pictures don't depict the sentence meanings, but rather some referents mentioned or otherwise potentially alluded to in the sentence.
  \item This is relevant because if you ask me to choose between pictures A and B in Figure 3 for the (obviously non-exhaustive) English sentence ``This morning Esther drank, among other things, coffee'', there are clear arguments in favour of either picture: B because it depicts some other things besides coffee, A because it depicts coffee (which Esther certainly drank) and doesn't depict water and fruit juice (which Esther may have drunk, but may not; perhaps the ``other things'' were in fact tea and champagne). Thus I wouldn't confidently interpret the choice of A as evidence of a persistent exhaustive interpretation of coffee.
\end{itemize}

\section{Reviewer 3}

\textbf{1. Comparability of the conditions}

The contrastive focus condition differs from the other two in non-trivial ways. Crucially, it has no "As for..." introduction of a topic. In the other two conditions a topic is introduced by “As for…”, and this topic delimits the range of alternatives that the focus of the sentence invokes.

This is a problematic confound for at least two reasons. First, topics guide attention and activate semantic fields that are not activated without them. This effect enters the processing of sentences in uncontrolled ways in the experiment, which is problematic in the case of the reading times and eye tracking measures.

Second, in the specific case in (6) and in examples like it, the introduction of the topic is likely to influence the interpretation of the comment that follows it. Out of context, in the absence of a verbal prefix the comment may be interpreted either as narrow focus (on the pre-verbal object) or as broad, VP-focus. The topic 'as for drinks' raises the likelihood of a narrow focus reading in the case of (6a) and (6b). As there is no such topic is (6c), a broad focus interpretation is readily available there. This confounds any comparisons that involve this condition, including both online and offline measures.

If I understood correctly, half of the items had a verbal prefix IN EACH CONDITION. If so, that half of the items in the contrastive condition got disambiguated syntactically by the post-verbal placement of this prefix. That may (at least to some degree) counteract the absence of the topic in the ‘contrastive’ condition for the purposes of the offline interpretation task.

But even so, the effect of the absence vs the presence of the topic is quite unlikely to be neutralized in online processing measures.

In short, the confounding difference in the information structural makeup of the first two conditions vs the third condition makes them incomparable.”

\textbf{2. Participant assignment}

I'm not sure I understand the design of how participants were assigned to different “parts” of the experiment, but if I understand it correctly, the arrangement is quite unusual. It sounds as if the authors had run an experiment third the size of what they call their experiment, but this smaller experiment was conducted three times as many times. This is due to the author’s decision to allow participants to take part in the smaller experiment (which the authors call a “part” of the experiment) two times or three times, each time getting one third of the full set of items (which are better called lists).

I am assuming that each of the three “parts”/lists contained the same number of items from each test condition and the same number of fillers in a Latin square design. At least, that's what the last sentence on page 7 suggests (though that is not completely clear either).

Allowing participants to complete varying numbers of lists invalidates the Latin square design for several reasons.

The primary goal of creating three distinct lists in a Latin square design is to prevent within-subject repetition of target items. If a participant completes two or three lists, s/he will encounter the exact same lexicalization multiple times across different conditions. This introduces repetition priming, carryover effects and hypothesis awareness, and it does so in an unbalanced way, since some participants took one list, some took two, and some took three. Choosing to participate more times (getting presented with more lists) also introduces self-selection bias.

Combining these data in a single standard LMM breaks statistical assumptions regarding equal exposure. Participants who do 108 trials (three lists) experience fatigue and
and task adaptation that participants doing 36 trials (one list) don't. The residual variance for three-list participants will likely be much higher (or systematically drift over time) compared to one-list participants. But standard LMMs assume homoscedasticity of residuals across all subjects.”

“Missing at Random (MAR)”

“This design flaw essentially contaminates the authors’ data, and this is especially relevant to reading times and eye-tracking, since second and third exposures completely alter natural reading and processing behavior.”

“They can discard all second- and third-exposure data, and run the ‘one-third experiment’ in a Latin square design with more participants (two times as many are needed as are left after discarding contaminated data).”

“If the ‘one-third sized’ experiment is too short (if one would need more data points per participant than that allows), then the authors may need to rerun the experiment so that all participants see all the material (all three lists).

\textbf{3. The task}

Participants were told that in each trial, after reading the stimulus sentence they should select the picture that best matches the sentence. As in the ‘only’-PVF sentences only a single object is referred to, multi-object images do not "match" the "meaning of the sentence". The same holds of unmodified-PVF sentences. As opposed to these two, in contrastive PVF sentences reference is made to multiple objects (through the addition of ‘among others’).

**Crucially, this alone may explain, on the one hand, the key differences between the last condition on the one hand, and the first two conditions on the other, as well as the key similarities between the first two conditions.**

In particular, the first key result of the experiment was the high rate of single-object image selections not only in the ‘only’-PVF condition, but also in the unmodified PVF condition, in clear contrast to the ‘among others’-PVF condition. This is not surprising, since stimulus sentences in the first two conditions only refer to a single object, while in the contrastive condition the sentence refers to multiple objects (in example (6c), “This morning Esther drank coffee, among other things”).

Further, while sentences in the contrastive condition made reference to multiple objects, they included explicit mention of just one object (coffee in the case of (6c)). The task instruction to choose the image that best matches the "meaning of the sentence" **may be interpreted in at least two ways.** First, the better matching image may be the one that contains the single object that is explicitly mentioned in the sentence.

“Second, the better matching image may well be considered to be the one that contains multiple objects, as ‘among others’ sentences made reference to multiple (at least three) objects: the one that is explicitly mentioned and a plurality (so, at least two) of ‘others’. This may explain another key result, specifically, that in the ‘among others’ condition in more than half of the cases participants selected the multiple-object (‘non-exhaustive’) image, but in more than one third of the cases they opted for the single-object (‘exhaustive’) picture. That may have been the function of how the instruction to select the image that “best matches the meaning of the sentence” is interpreted.

In fact, this seems a more plausible explanation for the relevant result than what the authors propose.

(What they claim is that this pattern of image-selection in the ‘among others’ condition is due to the robustness of the exhaustive interpretation associated with the PVF position. That seems neither likely nor coherent: the exhaustive, single-object interpretation is simply incompatible with the meaning of the core proposition the sentence conveys. In the case of (6c), for instance, this core proposition entails that Esther drank several (kinds of) drinks (at least three, as noted above). The sentence, then, cannot at the same time be interpreted as entailing that she only drank one (kind of) drink, which would be the exhaustive interpretation of PVF.)

In brief, the formulation of the task itself raises serious concerns regarding task validity.

Because the methodological concerns outlined above affect the core findings of the study, I have limited my review to these high-level issues for now. Addressing these fundamental points will likely require substantial changes to content, potentially leading to a very different paper. My recommendation is a resubmission, and I look forward to evaluating a thoroughly revised manuscript that addresses these primary concerns.
...
One of the key issues around Hungarian pre-verbal focus is whether it is inherently exhaustive, or if exhaustivity is a pragmatic inference. In other words, does the PVF construction itself entail that the focused element is the only one for which the predicate holds true, or can it be interpreted in a contrastive way, where the focused element stands in opposition to other alternatives without necessarily excluding them? For example, in (\ref{ex:pvf}) above, does this sentence imply that Anna ate only a banana \textit{and nothing else} -- an exhaustive reading, or could it mean that Anna ate a banana \textit{and not something else} (or even, that she ate a banana but maybe \textit{she also ate something else}) -- a contrastive reading? We are interested in the `default' interpretation of focus constructions such as these, and whether Hungarian speakers tend to understand them exhaustively or contrastively without additional context.
...
\begin{figure}[!ht]
\FIG{\includegraphics[width=1.0\textwidth]{figs/demographics_combined_donuts.png}}
{\caption{Demographic information of the participants in the experiment: gender (left) and age distributions (right)}
\label{demographics}}
\end{figure}

We recruited 55 native Hungarian speakers through Prolific (\url{www.prolific.com}). Participants were pre‑screened to confirm Hungarian as their first language and were paid approximately £7.50 per hour for their time. Prolific is an online recruitment platform that lets researchers post studies and invite eligible participants from its pool; interested individuals can then join from their own home using their own device, and they are redirected to the researcher’s online experiment hosted elsewhere. After completing the study, participants receive a completion code to submit on Prolific, which registers their participation as a ``submission'' and notifies the researcher for verification and payment. Participants' demographic information is pre-recorded on the platform and it is accessible to the researcher once a submission is accepted. After rejecting 3 participants (due to overly long submission time, failing attention checks, or missing ET data), the final sample consisted of 52 individuals (32 female; mean age = ~35). 

\subsection{Materials}

Our stimuli consisted of 54 experimental sentences (18 items × 3 conditions) with 54 additional filler sentences in a randomized order (108 in total), but making sure that the same items never followed each other. All sentences use verb phrases with focused direct object nouns in the accusative. Regarding lexical variation we aimed to devise a balanced set and used verbs both with and without verbal prefixes (50-50\%), included instances of both definite and indefinite objects (44.4-55.6\%), and used both countable and uncountable nouns. 
Verbal prefixes are very common and can act as perfective aspect markers, or modify the meaning of a verb, e.g. \textit{enni} vs \textit{megenni} `to eat' vs `to eat sth (up, until eaten)' or \textit{menni} vs \textit{kimenni} `to go' vs `to go out'. They are connected to telicity, without a verbal prefix example (\ref{ex:pvf}) for instance could be rendered in the past continuous: ``Anna was eating an apple.'' but without suggesting whether the action was ever completed or not. The main characteristic of verbal prefixes is that they can detach from the main verb if a constituent takes the preverbal focus position, but in our setup, they do not affect the sentences: verbal prefixes are all detached and appear immediately after the verb, as seen in (\ref{ex:pvf}), and they were presented in the same region during trials. For a summary of verbal prefixes in view of syntactic focus, see \citet[][50-61]{day_2021_role}. Definite direct objects in Hungarian require the use of specific verbal forms called the definite conjugation, one should think of this as agreement between the object and the verb. Neither these, nor the countability of certain nouns should affect the experiment regarding considerations of exhaustivity.

\hl{\textbf{DONE? Linguistic-material decisions need justification.} Explain the significance of verbal prefixes, definite versus indefinite conjugation, and countable versus uncountable nouns so decisions are comprehensible to non‑Hungarian readers. Since the introduction presents preverbal focus partly via separation of the verbal prefix from the verb, clarify how stimuli without verbal prefixes fit the design.}

To balance sentence length and make the targeted focus constructions appear not in the beginning of trials, we added some simple contextual fillers to the beginning of each sentence, such as \textit{ami X-t illeti} `regarding X' and time or place adverbials When presenting the sentences on the screen, we divided them into regions, and articles were kept together with nouns, so all three conditions had the same number of regions (7), and the focused element was always in region 6 (r6). The stimuli were created by changing a set of 18 simple transitive sentences by modifying the base sentence with the addition of the focus marker \textit{csak} `only' for the exhaustive condition, and the phrase \textit{többek között} `amongst others' for the contrastive condition.

To make sure that sentences in the contrastive condition are not awkward, we ran a quick acceptability judgment study on all 18 target items using a survey with a 7 point Likert-scale, and 18 fillers. Participants were native speakers who grew up and were educated in Hungary (\(n=15\); female=7; mean age was \(37\) [SD \(=11\)]). The mean acceptability rating was \(\bar{x}=5.65\) (SD \(=0.45\)). \hl{DONE Acceptability judgement}
...
The three conditions were: a) exhaustive focus with \emph{csak} `only' (e.g., (\ref{ex:stimuli_exhaustive})), b) unmodified, simple pre-verbal focus (PVF) (e.g., (\ref{ex:stimuli_pvf})), and c) contrastive focus with \emph{többek között} `amongst others' (e.g., (\ref{ex:stimuli_contrastive})). Each item was paired with two images: type A depicting a single-object -- the exhaustive reading -- and type B depicting a multi-object variation -- the contrastive reading. See Figure~\ref{fig:stimuli_8} for an example of the image pairs accompanying the sentences in the experiment.

\hl{\bigstar \textbf{Concerns about stimulus design and procedure.} The exhaustive and unmodified conditions begin with expressions such as ``regarding drinks/fruits'', which evoke alternatives and may bias contrastive or exhaustive interpretations. The contrastive condition also differs from the other two in sentence structure, discourse framing, and lexical content, creating a possible confound: observed differences may reflect sentence context, lexical material, or length rather than focus interpretation.}  
...
{\caption{Image pair for the sentences in examples (\ref{ex:stimuli_exhaustive}--\ref{ex:stimuli_contrastive}). The left image (type A) corresponds to the exhaustive reading, while the right image (type B) corresponds to the contrastive reading.}
\label{fig:stimuli_8}}
\end{figure}
...
\glb which{\sc [rel]} {the drink-{\sc acc}} regard-3{\sc sg} Eszter tday coffee-{\sc acc} drink-{\sc pst}-3{\sc sg}//
...
Every experiment had four attention checks with sentences of similar length such as: `This sentence is not real, please choose the potato.' followed by an image choice of a potato and a tomato, or `This sentence is an attention test, choose the smaller animal here.' with an image choice between an ant and a polar bear, or `If you are paying attention to what you are reading, choose the bird.' with a choice between two words corresponding to `pelican' and `tiger', or `Are you paying attention? If yes, which one is the last meal of the day?' with a choice between two words corresponding to `breakfast' and `dinner'. The first two conforms to the style and presentation of our 54 experimental items, the latter two follows those of the 54 fillers, using spelled out words on an image instead of pictures.

\hl{\textbf{Describe attention checks.} Provide details about the attention‑check items used. DONE}

To avoid overly long experimental sessions, we split the experiment into three equal parts, meaning that three separate studies were posted on Prolific. Each consisted of 40 trials (36 experimental items + 4 attention checks). Invited participants could complete one, two, or all three parts as separate submissions one after another in one sitting, or across separate days in multiple sittings if there were open slots still; participation times and location were entirely up to each individual according to their own convenience. This means that we had three participant groups that could overlap, and some participants completed only 36, some, 72, and some all 108 items. The three parts were split in a way that for each group each item only appeared in one condition, and each session contained four attention check questions intermixed and evenly distributed across the trials. Participants never saw the same exact stimuli twice, as the three parts had no repetitions. The main reason behind this decision was that for some people calibrating the eye tracker proved to be difficult, and we did not want participants to get frustrated and abandon the experiment if it lasted too long. This solution was also necessary because of the relatively small participant pool of Hungarian L1 speakers who also matched the technical requirements. This setup resulted in 72 valid submissions (25 for part one, 23 for part two, and 24 for part three) from 55 individuals, and thus every item was interacted with at least 23 times. 

\hl{ \bigstar \textbf{Model repeated submissions explicitly}. If the same participant could contribute multiple submissions, this must be modelled, especially if participants might have seen the \textbf{same items in different conditions}, because repeated exposure affects reading and reaction times.??}



\subsection{Design}

We built a sentence--picture matching (SPM) task, in which participants read a sentence and were asked to select the best fitting meaning-representation between two visual stimuli. First, participants saw a preview of the image pair to get familiar with the objects/context, on the left and right half of the screen (20\% of the screen's width and 40\% of the screen's height) on a canvas 50\% larger than the images to mitigate the inaccuracies of the eye tracker. After pressing the spacebar they were presented with the sentence word by word (region by region) in the middle of the screen in a self-paced reading (SPR) manner, in which participants had to use the spacebar to advance through the sentences. We recorded reading times throughout. After reading through the sentence, the image pair re-appeared and participants had to select the image that best matched the meaning of the sentence they just read. The choice had to be made with a mouse-click anywhere on the corresponding canvas (left or right), and reaction times (RT) and eye movements were recorded during this picture matching phase. The image types were counterbalanced throughout the trials regarding their position of the left or right side of the screen. The segments were separated by 1000 ms pauses, and after the choice, the next trial started after a brief (1000 ms) pause. The experiment was designed to be completed in around 25-30 minutes, and participants had to complete it in one sitting. See Figure~\ref{fig:design} for a visual representation of the experiment design.

\begin{figure}[!ht]
\FIG{\includegraphics[width=1.0\textwidth]{figs/design.png}}
{\caption{Design}
\label{fig:design}}
\end{figure}
...
\hl{\textbf{NEW:} Statistical analysis of the binary image choice. DONE.}

To evaluate the effect of sentence type on image selection, we fitted a mixed-effects logistic regression model. The model predicted the binary image Choice based on the fixed effect of Condition. We applied sum-to-zero contrast coding for the condition variable and included random intercepts for both participants and items to account for repeated measures: \texttt{Choice \sim~ \text{Conditon} + (1 \mid~\text{Participant}) + (1 \mid~\text{Item})}. 
Across all experimental conditions, participants exhibited an overall baseline preference for image type A over image type B ($\beta = 2.62$, $SE = 0.30$, $z = 8.65$, $p < .001$, $OR = 13.72$). Relative to the grand mean across all conditions, the likelihood of choosing image A was significantly elevated in both the exhaustive condition ($\beta = 1.54$, $SE = 0.20$, $z = 7.86$, $p < .001$, $OR = 4.64$) and the unmodified condition ($\beta = 1.78$, $SE = 0.21$, $z = 8.63$, $p < .001$, $OR = 5.93$). Conversely, the contrastive condition strongly decreased the odds of selecting image A relative to the overall mean ($\beta = -3.32$). Considerable individual variance was observed across participants ($\sigma^2 = 3.60$, $SD = 1.90$), whereas variation across experimental items was modest ($\sigma^2 = 0.24$, $SD = 0.49$). Post-hoc pairwise comparisons using Holm's adjustment revealed that participants in the exhaustive condition were significantly more likely to select image A compared to those in the contrastive condition ($\beta = 4.85$, $SE = 0.34$, $z = 14.46$, $p < .0001$). Similarly, the unmodified condition elicited a significantly higher likelihood of choosing image A than the contrastive condition ($\beta = 5.10$, $SE = 0.35$, $z = 14.38$, $p < .0001$). We observed no statistically significant difference in choice behavior between the exhaustive and unmodified conditions ($\beta = -0.24$, $SE = 0.35$, $z = -0.70$, $p = 0.484$). See Table~\ref{tab:choice_stats} for a summary of the model results.

\begin{table}[ht]
\centering
\caption{Mixed-effects logistic regression results for binary image choice (upper) \& Pairwise comparisons of choice probabilities by condition (lower)}
\label{tab:choice_stats}
\begin{tabularx}{\textwidth}{@{}Xrrrrr@{}}
\toprule
Parameter & Estimate & SE & $z$-value & $p$-value \\
\midrule
(Intercept) (Contrastive) & 2.62 & 0.30 & 8.65 & $<.001$ & \\
Condition1 (Exhaustive) & 1.54 & 0.20 & 7.86 & $<.001$ & \\
Condition2 (Unmodified) & 1.78 & 0.21 & 8.63 & $<.001$ & \\
\bottomrule
\multicolumn{6}{p{\textwidth}}{\footnotesize \textit{Note:} Estimates are given on the log-odds scale. The model utilized sum-to-zero contrast coding.}
% {\footnotesize \textit{Note:} Estimates are given on the log-odds scale. The model utilized sum-to-zero contrast coding.}
\end{tabularx}
...
\midrule
Exhaustive -- Unmodified  & -0.25 & 0.35 & -0.70 & .4841 & \\
Exhaustive -- Contrastive &  4.85 & 0.34 & 14.46 & $<.0001$ & \\
Unmodified -- Contrastive &  5.10 & 0.35 & 14.38 & $<.0001$ & \\
\bottomrule
\multicolumn{6}{p{\textwidth}}{\footnotesize \textit{Note:} Estimates are presented on the log-odds ratio scale. $p$-values are adjusted using Holm's method for 3 tests.}
% {\footnotesize \textit{Note:} Estimates are presented on the log-odds ratio scale. $p$-values are adjusted using Holm's method for 3 tests.}
\end{tabularx}
\end{table}
...
\begin{figure}[ht]
\FIG{\includegraphics[width=1.0\textwidth]{figs/choice_type_per_condition.png}}
...
In short, the behavioural results show that participants overwhelmingly interpreted the unmodified pre-verbal focus (PVF) sentences like exhaustive \textit{only}-sentences, choosing type A images. However, in case of contrastive \textit{amongst others}-sentences, people preferred the type B images, exhibiting a tendency toward non-exhaustive readings, but with almost 37.7\% of the choices -- somewhat unexpectedly -- still being cast on the exhaustive interpretation type A images (see Figure~\ref{fig:choice}). This suggests that even with the presence of a phrase that licenses an inclusive, contrastive meaning, the exhaustivity effect is still quite salient. This pattern provides clear evidence that the PVF construction is not obligatorily interpreted as exclusively exhaustive; a non-exhaustive interpretation remains available. Our results further show that a contrastive reading can be elicited readily, and that the presence of an inclusive, contrastive licensing phrase does not fully eliminate the construction’s exhaustive bias.
...
We analyzed the reading times during self-paced reading and reaction times (RT) during the picture matching phases, see the plots in Figure~\ref{fig:region_rt_by_choice_with_choice_rt_split_by_choice} for an overview. Reading times and decision reaction times were winsorized by replacing values above the mean +2 SD with the mean to mitigate the impact of extreme outliers. In the following analyses, values were log-transformed using $\log(1+x)$ to stabilize variance.

\begin{figure}[ht]
\FIG{\includegraphics[width=1.0\textwidth]{figs/region_rt_by_choice_with_choice_rt_split_by_choice.png}}
...
For sentence reading times, the Type III Wald chi-square tests showed a significant main effect of Condition, $\chi^2(2)=18.66$, $p<.001$, but no main effect of Choice, $\chi^2(1)=0.06$, $p=.806$, and no interaction between Condition and Choice, $\chi^2(2)=2.10$, $p=.351$ (see Table~\ref{tab:sentence-rt}). This indicates that the focus condition influenced how long participants took to read the sentences, but the interpretation they ultimately settled on -- type A or type B -- did not significantly affect reading times, and there was no interaction between the two factors. 

\begin{table}[htbp]
\centering
\caption{Fixed-effects tests for whole-sentence reading times, compared to \textit{exhaustive} condition and type A image choice as reference levels}
...
Post-hoc pairwise comparisons revealed that the significant effect of Condition was driven by significantly faster reading times in the exhaustive condition compared to the unmodified condition (\textit{p}<.001 A--A) when participants chose the expected type A image, but not when they chose the type B image (\textit{p}=.994 B--B), see Table \ref{tab:sentence-rt-pairwise-within-choice}. There was no significant difference in whole sentence reading times between the exhaustive and contrastive conditions regardless of the choice type (\textit{p}=.149 A--A, \textit{p}=.998 B--B), nor between the unmodified and contrastive conditions (\textit{p}=.995 A--A; \textit{p}=.876 B--B). There was also no significance accross choice types within any of the conditions (see Table \ref{tab:sentence-rt-pairwise-across-choice}).

Taken together, this pattern of results suggests that the exhaustive focus marker \textit{csak} `only' facilitated processing relative to the unmodified PVF construction, but this advantage was limited to trials in which participants made the expected type A choice. This indicates that the processing benefit of exhaustive marking emerged primarily when participants’ interpretations aligned with the intended exhaustive reading, and did not extend to trials in which they selected the type B image. At the same time, the contrastive condition with \textit{többek között} `amongst others' did not differ significantly from either the exhaustive or the unmodified condition, regardless of choice type, suggesting that the contrastive modifier did not reliably affect overall reading times. Overall, any processing advantage associated with exhaustive marking appears to have been constrained rather than general. See Figure~\ref{fig:sentence_log_rt_by_condition_split_by_choice} for a visual representation of the whole-sentence reading times by condition, split by choice type.

% facilitate
% expected

\begin{figure}[htbp]
\FIG{\includegraphics[width=1.0\textwidth]{figs/sentence_log_rt_by_condition_split_by_choice.png}}
...
\begin{figure}[htbp]
\FIG{\includegraphics[width=1.0\textwidth]{figs/log_choice_rt_by_condition_split_by_choice.png}}
...
The within-choice pairwise comparisons (Table~\ref{tab:total-dwell-pairwise-within-choice}) show that, among participants who chose the type~(A) image, total dwell time was significantly higher in the contrastive condition than in the exhaustive condition ($\hat\beta = -0.1876$, $SE = 0.0531$, $t(1258) = -3.530$, $p = .0057$), with a marginal difference also between the contrastive and unmodified conditions ($\hat\beta = -0.1402$, $SE = 0.0529$, $t(1256) = -2.650$, $p = .086$). The exhaustive and unmodified conditions did not differ for A-choices ($p = .808$). This pattern indicates that choosing the type A image required more deliberation when the sentence was in the contrastive condition, reflecting the cognitive effort of committing to an exhaustive interpretation in the face of an explicit contrastive cue. Among participants who chose the type B image, no significant between-condition differences were observed (all $p \ge .232$), suggesting that the decision effort for the non-exhaustive choice was broadly similar across conditions.
...
\begin{figure}
\FIG{\includegraphics[width=1.0\textwidth]{figs/log_total_fixation_by_condition_and_choice.png}}
...
Second, the forced-choice, sentence--picture matching paradigm we used requires participants to commit to one of two interpretations and does not capture gradient or intermediate representations. The 37.7\% exhaustive choices in the contrastive condition, for example, could reflect either genuine exhaustive interpretations or a residual structural bias that the paradigm forces into a categorical outcome. Future work using magnitude estimation, acceptability rating, or more fine-grained response measures could help disentangle these possibilities.
...
\hl{Stimuli, full results, replication data and code} can be found at...
...
\# & A & B & \# & A & B \\
1 & \includegraphics[height=.096\textheight]{figs/1a.png}  &
\includegraphics[height=.096\textheight]{figs/1b.png}  &
2 & \includegraphics[height=.096\textheight]{figs/2a.png}  &
\includegraphics[height=.096\textheight]{figs/2b.png}  \\
3 &\includegraphics[height=.096\textheight]{figs/3a.png}  &
\includegraphics[height=.096\textheight]{figs/3b.png}  &
4 & \includegraphics[height=.096\textheight]{figs/4a.png}  &
\includegraphics[height=.096\textheight]{figs/4b.png}  \\
5 & \includegraphics[height=.096\textheight]{figs/5a.png}  &
\includegraphics[height=.096\textheight]{figs/5b.png}  &
6 & \includegraphics[height=.096\textheight]{figs/6a.png}  &
\includegraphics[height=.096\textheight]{figs/6b.png}  \\
7 & \includegraphics[height=.096\textheight]{figs/7a.png}  &
\includegraphics[height=.096\textheight]{figs/7b.png}  &
8 & \includegraphics[height=.096\textheight]{figs/8a.png}  &
\includegraphics[height=.096\textheight]{figs/8b.png}  \\
9 & \includegraphics[height=.096\textheight]{figs/9a.png}  &
\includegraphics[height=.096\textheight]{figs/9b.png}  &
10 & \includegraphics[height=.096\textheight]{figs/10a.png} &
\includegraphics[height=.096\textheight]{figs/10b.png} \\
11 & \includegraphics[height=.096\textheight]{figs/11a.png} &
\includegraphics[height=.096\textheight]{figs/11b.png} &
12 & \includegraphics[height=.096\textheight]{figs/12a.png} &
\includegraphics[height=.096\textheight]{figs/12b.png} \\
13 & \includegraphics[height=.096\textheight]{figs/13a.png} &
\includegraphics[height=.096\textheight]{figs/13b.png} &
14 & \includegraphics[height=.096\textheight]{figs/14a.png} &
\includegraphics[height=.096\textheight]{figs/14b.png} \\
15 & \includegraphics[height=.096\textheight]{figs/15a.png} &
\includegraphics[height=.096\textheight]{figs/15b.png} &
16 & \includegraphics[height=.096\textheight]{figs/16a.png} &
\includegraphics[height=.096\textheight]{figs/16b.png} \\
17 & \includegraphics[height=.096\textheight]{figs/17a.png} &
\includegraphics[height=.096\textheight]{figs/17b.png} &
18 & \includegraphics[height=.096\textheight]{figs/18a.png} &
\includegraphics[height=.096\textheight]{figs/18b.png} \\
\end{tabular}
}
{\caption{Complete visual stimulus set with single- (type A) and multi-entity (type B) image pairs}}
...
% @mastersthesis{day_2021_role,
%   title = {{The Role and Function of Hungarian Verbal Prefixes: An Overview}},
%   shorttitle = {{The Role and Function of Hungarian Verbal Prefixes}},
%   author = {Day, Dallin},
%   year = 2021,
%   address = {Bloomington, IN},
%   url = {https://www.proquest.com/docview/2572567389/abstract/752F1DEFCAEC4BC3PQ/1},
%   urldate = {2026-09-23},
%   abstract = {This thesis examines the various debates surrounding the contentious subject of verbal prefixes in Hungarian by comparing sources from the leading authors in Hungarian linguistics. Literature on their relationship to the verb is presented, as well as intra- and interlingual sources that inform their origin. A number of other topics are presented, including aspect, stress, and the ways in which verbal prefixes can impact argument structure. Verbal prefixes are then shown to be equivalent with a larger class of verb modifiers. Scholarship which places verb modifiers within the syntax is reviewed, especially regarding the likewise-contentious focus position. Not all the debates presented are settled as yet. One of the main goals of the thesis is to contextualize Hungarian linguistic debates in order to inform potential research in the language. For example, consensus on the origins and general aspectual function of verbal prefixes has mostly been reached. Some debate still exists, however, on various impacts of the prefix on the rest of the sentence. The thesis concludes that prominent areas for research still exist, especially in the realms of reduplication, argument structure modification, and syntax.},
%   copyright = {Database copyright ProQuest LLC; ProQuest does not claim copyright in the individual underlying works.},
%   isbn = {979-8-5381-3321-5},
%   langid = {french},
%   school = {Indiana University},
%   keywords = {Aspect,Dictionaries,Hungarian,Language,Linguistics,Orthography,Preverbs,Semantics,Syntax},
%   file = {C:\Users\parti\Zotero\storage\DWV4JRZE\Day - 2021 - The Role and Function of Hungarian Verbal Prefixes, An Overview - Indiana University.pdf}
% }

\bibitem[Day (2021)]{day_2021_role}
Day, Dallin. 2021. \textit{The Role and Function of Hungarian Verbal Prefixes: An Overview.} Master's thesis. Bloomington, IN: Indiana University.
